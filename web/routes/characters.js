...
// POST /characters/:char_id/update-name  (renames all rows that share the same char_name)
router.post('/characters/:char_id/update-name', express.urlencoded({ extended: false }), async (req, res) => {
  ...
  req.session.flash = `✅ Character renamed to ${nameCap}.`;
  res.redirect('/characters');
});

// POST /characters/:char_id/update-realm  (updates realm for all rows in the group)
router.post('/characters/:char_id/update-realm', express.urlencoded({ extended: false }), async (req, res) => {
  if (!requireLogin(req, res)) return;
  ...
  req.session.flash = `✅ Realm updated for ${char.char_name}.`;
  res.redirect('/characters');
});

// POST /characters/:char_id/update-class  (updates char_class for all rows in the group)
router.post('/characters/:char_id/update-class', express.urlencoded({ extended: false }), async (req, res) => {
  if (!requireLogin(req, res)) return;
  ...
  req.session.flash = `✅ Class updated for ${char.char_name}.`;
  res.redirect('/characters');
});
